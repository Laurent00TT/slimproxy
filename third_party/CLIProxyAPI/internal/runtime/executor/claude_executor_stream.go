...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	go func() {
		...
		defer func() {
			...
			// slimproxy patch (see SLIMPROXY_PATCHES.md): a stream that ends
			// without ever scanning a usage line or hitting a scanner error
			// published nothing -- the request vanishes from the books while
			// the access log records a 200. Same backstop, same defer shape,
			// as openai_compat_executor.go's stream goroutine.
