...
- 不改 third_party fork 的任何行为：本设计只使用 fork 已经传进
  `HandleUsage(ctx, record)` 的请求作用域 ctx（旁证：同一 ctx 上
  `GetResponseHeaders` 已在取回本请求的响应头）。
...
- usage 发布由流尾触发且经异步队列，最后一笔客户端写可能未计入
  `wb_ms`：原子读快照，少算不多算。
- ctx 必须是请求 ctx——以 ResponseHeaders 机制为既有证据，另加 wiring
  测试锁定（publish 时能从 ctx 取到本请求的 NetTimings）。
...
- netmeter 单测：分块慢读 body → `up_ms` 反映延迟；阻塞式 writer 桩 →
  `wb_ms` 反映阻塞；handler panic 时包装不泄漏。
- wiring 测试（`journal_wiring_test.go` 风格）：完整 gin + collector +
  假 usage 发布，断言同一条 req 事件同时带三段。
...
- fork 升级时 `PublishRecord` 若改为断开 ctx 传递，三段静默消失——
  wiring 测试即为此设的哨兵，升级流程照 SLIMPROXY_PATCHES.md 走。
