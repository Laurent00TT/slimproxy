...
// leaked in, and the three rated rows are chosen so a mean of per-row rates
// (2428 KB/s) differs from the ratio of sums (700 KB/s). The floor is
// inclusive -- -stats prints it as "≥100ms" -- so a leg of exactly
// MinRatedUploadMs is rated and one a millisecond shorter is not.
func TestSummariseUploadThroughput(t *testing.T) {
	...
	s := Summarise([]Event{
		...
		{Kind: KindRequest, OK: &ok, InKB: 498, UploadMs: MinRatedUploadMs},      // 计入：恰在下限上，4980KB/s
	...
	if s.UploadRated != 3 {
		t.Fatalf("UploadRated = %d, want 3（下限含等号：恰为 %dms 的行也计入）", s.UploadRated, MinRatedUploadMs)
	}
	if want := 3570.0 * 1000 / 5100; s.UploadKBps != want {
