...
- **`stream-idle-timeout` 负责切断死掉的流。** 一个流式响应在窗口内（默认 90 秒）一个
  ...
  PING、30 秒无回应即断开，所以真正死掉的连接即使关了看门狗，也会在最后一帧之后约
  60 秒内失败。每次这样断开一条仍承载请求的连接，都会带上该请求的 ID 记一条 warn
  （`utls: health check closed the connection …`），探活切断请求的频率由此可数。只有
  看门狗能抓住的，是连接还在、上游却不再发东西的流——关掉它，这种情况照旧无限期
  挂起。空闲连接上也会发这种 PING，而每个请求都会留下一条空闲连接（每个请求自建
  客户端，连接从不复用），所以引擎还会关掉连续 90 秒没承载请求的连接；否则这些
  PING 会让每一条都经 Clash 和节点永远开着。
...
`models` 目前是保留项：非空列表会被启动校验和 `check` 拒绝。
留空（`models: []`）会提供已加载凭据暴露的全部模型。旧版本虽然显示
「精确匹配白名单」，但服务路径从未执行这个限制；现在拒绝配置，避免误报访问控制。

真正实现白名单之前，仍需决定精确名称、日期后缀、思考后缀（`gpt-5.5(high)`）
怎样匹配。这是访问控制边界，策略仍在 `proxy/config.go` 里标着 `TODO(you)`；
执行限制应落在引擎中各客户端方言共用的模型解析处。
